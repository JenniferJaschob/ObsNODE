#Adaptation from https://github.com/konstantinhess/SCIP-Net/blob/main/src/models/timr_varying_model.py
#Konstantin Hess et al., "Stabilized Neural Prediction of Potential Outcomes in Continuous Time ", 
#Published as a conference paper at ICLR 2025 doi:  https://doi.org/10.48550/arXiv.2410.03514
...
try:
    # Optuna is an optional dependency; if present we can use a Bayesian TPE sampler
    # Newer Ray versions expose the Optuna search in `ray.tune.search.optuna`.
    # Fall back to the older `ray.tune.suggest.optuna` path for compatibility.
    try:
...
def train_eval_factual(args: dict, train_f: Dataset, val_f: Dataset, orig_hparams: DictConfig, input_size: int, model_cls,
                       tuning_criterion='rmse', one_step_counterfactual: bool = False, **kwargs):
    """
    ...
    if tuning_criterion == 'rmse':
        # If requested, compute one-step-ahead normalised RMSE for tuning
        val_rmse_orig, val_rmse_all = model.get_normalised_masked_rmse(val_f, one_step_counterfactual=one_step_counterfactual)
        # Use dict payload for Tune report to be compatible with different Ray versions
...
class TimeVaryingCausalModel(LightningModule):
    ...
    def get_masked_bce(self, dataset: Dataset):
        ...
        treatment_pred = torch.tensor(self.get_propensity_scores(dataset)).cpu()
        ...
        return float(bce_orig), float(bce_all)
    ...
    def get_normalised_masked_rmse(self, dataset: Dataset, one_step_counterfactual=False):
        ...
        nan_mask = ~np.isnan(mse.sum(axis=-1))
        # Calculation like in original paper (Masked-Averaging over datapoints (& outputs) and then non-masked time axis)
        mse_orig = mse[nan_mask].sum(0).sum(-1) / dataset.data['active_entries'][nan_mask].sum(0).sum(-1)
        mse_orig = np.nanmean(mse_orig)
        rmse_normalised_orig = np.sqrt(mse_orig) / dataset.norm_const

        # Masked averaging over all dimensions at once
        mse_all = mse[nan_mask].sum() / dataset.data['active_entries'][nan_mask].sum()
        ...
        if one_step_counterfactual:
            ...
            #nan_mask = ~np.isnan(mse_last)
            nan_mask = ~np.isnan(mse_last.sum(axis=-1))
            mse_last = mse_last[nan_mask].sum() / last_entries[nan_mask].sum()
    ...
    def get_normalised_n_step_rmses(self, dataset: Dataset, datasets_mc: List[Dataset] = None):
        ...
        #     mse_orig = mse[not_nan].sum(0).sum(-1) / dataset.data_processed_seq['active_entries'][not_nan].sum(0).sum(-1)
        # else:
        #     not_nan = ~np.isnan(dataset.data_processed_seq['outputs'])
        #     mse_orig = np.nansum(np.nanmean(mse, axis=-1), axis=0) / (dataset.data_processed_seq['active_entries'] * (not_nan.sum(axis=-1, keepdims=True)>0)).sum(0).sum(-1)
    ...
    def finetune(self, resources_per_trial: dict):
        ...
        # ray.init signature changed across Ray versions - private kwargs like `_redis_max_memory`
        # may not be accepted. Try to init with it first, otherwise fall back to a safer call.
        # Ensure worker processes can import local `src` package by setting PYTHONPATH
        try:
            project_root = get_original_cwd()
        except Exception:
            project_root = os.getcwd()

        common_ray_kwargs = dict(
            num_gpus=len(eval(str(self.hparams.exp.gpus))),
            num_cpus=8,
            include_dashboard=False,
            runtime_env={
                "working_dir": str(project_root),
                # Set PYTHONPATH inside the runtime environment so remote workers can import local modules
                "env_vars": {"PYTHONPATH": str(project_root)}
            },
        )
        try:
            ray.init(**{**common_ray_kwargs, "_redis_max_memory": ray_constants.FUNCTION_SIZE_ERROR_THRESHOLD})
        except Exception as e:
            logger.warning(f"ray.init with _redis_max_memory failed ({e}), retrying without that kwarg.")
            ray.init(**common_ray_kwargs)

        hparams_grid = {k: tune.choice(v) for k, v in sub_args['hparams_grid'].items()}

        # Optionally use a Bayesian optimizer (Optuna TPE) as search algorithm.
        # If Optuna is not installed, fallback to default random sampling via tune.choice.
        search_alg = None
        if _HAS_OPTUNA:
            try:
                ...
                search_alg = OptunaSearch(sampler=sampler,
                                         ...
                                         mode="min")
                # Limit concurrent suggestions to available resources to avoid duplicate samples
                #try:
                #    gpu_per_trial = resources_per_trial.get('gpu', 0) if resources_per_trial else 0
                #    avail_gpus = len(eval(str(self.hparams.exp.gpus)))
                #    max_concurrent = max(1, int(avail_gpus / (gpu_per_trial if gpu_per_trial > 0 else 1)))
                #except Exception:
                #    max_concurrent = 1
                max_concurrent = 1
                search_alg = ConcurrencyLimiter(search_alg, max_concurrent=max_concurrent)
                logger.info(f"Using Optuna TPE search with max_concurrent={max_concurrent}")
            except Exception as e:
                logger.warning(f"OptunaSearch initialization failed ({e}), falling back to random sampling.")
                search_alg = None
        else:
            logger.info("Optuna not available - using random sampling (tune.choice).")

        # Optionally run tuning using only one-step-ahead RMSE if configured
        try:
            one_step_flag = bool(getattr(self.hparams.exp, 'hpo_one_step', False))
        except Exception:
            one_step_flag = False

        # Ensure Tune writes results inside the project and does not raise on failed trials
        local_ray_results = os.path.join(project_root, "outputs", "ray_results")
        ...
        analysis = tune.run(tune.with_parameters(train_eval_factual,
                                                 ...
                                                 bce_weights=self.bce_weights if hasattr(self, 'bce_weights') else None,
                                                 projection_horizon=self.projection_horizon
                                                 if hasattr(self, 'projection_horizon') else None,
                                                 one_step_counterfactual=one_step_flag),
        ...
        # Persist best hyperparameters to disk: merge into a single file per dataset
        try:
            try:
                project_root = get_original_cwd()
            except Exception:
            ...
            dataset_name = getattr(self.hparams.dataset, 'name', None) or 'dataset'
            tuned_fn = f"hparams_tuned_{dataset_name}.yaml"

            cfg_dir = os.path.join(project_root, 'config', 'backbone', 'scip_hparams')
            os.makedirs(cfg_dir, exist_ok=True)
            primary_path = os.path.join(cfg_dir, tuned_fn)

            # Load existing file (if any) and merge the new model entry
            try:
                if os.path.exists(primary_path):
                    existing_cfg = OmegaConf.load(primary_path)
                    existing = OmegaConf.to_container(existing_cfg, resolve=True)
                else:
                    existing = {}
            except Exception:
                existing = {}
            ...
            # Store best config under model.<model_type>
            existing['model'][self.model_type] = analysis.best_config

            OmegaConf.save(OmegaConf.create(existing), primary_path)

            # Also save a copy into current working directory (Hydra run dir) for convenience
            try:
                run_path = os.path.join(os.getcwd(), tuned_fn)
                OmegaConf.save(OmegaConf.create(existing), run_path)
            except Exception:
                run_path = primary_path

            logger.info(f"Saved best hyperparameters to '{primary_path}' and '{run_path}'.")
        except Exception as e:
            logger.warning(f"Failed to persist best hyperparameters: {e}")
