#Adaptation from https://github.com/konstantinhess/SCIP-Net/blob/main/src/models/scip.py
...
class SCIPEncoder(SCIP):
    ...
    def prepare_data(self) -> None:
        if getattr(self, 'data_already_prepared', False):
            return
        ...
        self.data_already_prepared = True
...
class SCIPDecoder(SCIP):
    ...
    def training_step(self, batch, batch_ind):
        ...
        if batch['outputs'].size(1) == 1:
            prev_nan = torch.isnan(batch['prev_outputs'])
            # prev_nan: [B, T, feat]. Reduce across feat -> [B, T, 1] to match active_entries
            if prev_nan.dim() == 3:
                mask = prev_nan.any(dim=-1, keepdim=True)
            else:
                mask = prev_nan.unsqueeze(-1)

            # Zero mse_loss entries where prev outputs are NaN. Expand mask to mse_loss last dim if needed.
            if mask.shape != mse_loss.shape:
                mask_exp = mask.expand(-1, -1, mse_loss.size(-1))
            else:
                mask_exp = mask

            mse_loss[mask_exp] = 0
            batch['active_entries'][mask] = 0
