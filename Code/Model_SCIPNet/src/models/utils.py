#Adaptation from https://github.com/konstantinhess/SCIP-Net/blob/main/src/models/utils.py
...
def forward_fill(arr):
    """
    Takes array of shape (n, m, d) and fills NaNs with last non-NaN value along axis 1
    """
    # mask = np.isnan(np.array(arr))
    ...
    # Loop over the last dimension and apply forward fill for each slice
    for i in range(d):
        mask = np.isnan(arr[:, :, i])
        idx = np.where(~mask, np.arange(m), 0)
        np.maximum.accumulate(idx, axis=1, out=idx)
        out[:, :, i] = arr[np.arange(n)[:, None], idx, i]
    # Replace remaining NaNs with 0 --> only if leading NaNs exist; masked away in loss calculation
    return torch.tensor(out).nan_to_num(0)
