#Adaptation from https://github.com/philippwendland/OptAB/blob/main/Code/OptAB/utils_paper.py
#Philipp Wendland et al., "An optimal antibiotic selection framework for Sepsis patients using Artificial Intelligence ", 
#in NPJ Digit Med. 2024 Nov 29;7(1):343.doi: 10.1038/s41746-024-01350-y. 
...
def train(model, train_output, train_toxic, train_treatments, covariables, active_entries, validation_output, validation_toxic, validation_treatments, covariables_val, active_entries_val, static = None,static_val = None, lr=0.001, batch_size=500, patience=10, delta=0.0001, max_epochs=1000, weight_loss=True,rectilinear_index=None,early_stop_path=None, mu=None,min_epoch=0):
    ...
    if static is not None:
        ...
        if train_toxic is not None:
            train_dataset = torch.utils.data.TensorDataset(train_coeffs, train_output, active_entries, train_toxic, train_treatments)
        else:
            train_dataset = torch.utils.data.TensorDataset(train_coeffs, train_output, active_entries,  train_treatments)
    ...
    for epoch in range(max_epochs):
        for batch in train_dataloader:
            ...
            if static is not None:
                ...
                if train_toxic is not None:
                    ...
                    batch_coeffs, train_output_b, active_entries_b,  train_treat_b = batch
                    train_toxic_b = None
                static_b=None
        ...
        if validation_toxic is not None:    
        ...
        if validation_toxic is not None: 
            ...
            loss_val=loss_output_val + mu * loss_a_val
        #
        if loss_val.isnan():
            return loss_val
...
def train_dec_offset(model,model_decoder,train_output, train_toxic, train_treatments, covariables, time_covariates, active_entries, validation_output, validation_toxic, validation_treatments, covariables_val, validation_time_covariates, active_entries_val, static,static_val, offset=0, max_horizon=50, lr=0.001, batch_size=500, patience=10, delta=0.0001, max_epochs=1000, weight_loss=True, rectilinear_index=None, offset_train_list=None, offset_val_list=None, early_stop_path=None, dec_expand=True,sofa_expand=True, mu=None,med_dec=False,med_dec_start=True, min_epoch  = 0):
    ...
    if static is not None:
        ...
        if train_toxic is not None:
            ...
            train_dataset = torch.utils.data.TensorDataset(train_coeffs, train_output, active_entries, train_treatments, time_covariates, train_treatments, covariables)
    ...
    for epoch in range(max_epochs):
        for batch in train_dataloader:
            ...
            if static is not None:
                ...
                if train_toxic is not None:
                    ...
                    batch_coeffs, train_output_b, active_entries_b,  train_treat_b, time_covariates_b, train_treatments_b, covariables_b = batch
                    train_toxic_b = None
                static_b=None
                        ...
                        if sofa_expand:
                            ...
                            if med_dec_start:
                                ...
                                if static is not None:
                                    ...
                                    zt_cov = torch.cat([zt_cov,sofa_start,treat_start,treat_past],axis=-1)

                            else:
                                if static is not None: zt_cov = torch.cat([zt_cov,sofa_start,static_b],axis=-1)
                                else: zt_cov = torch.cat([zt_cov,sofa_start],axis=-1)
                                
                        else:
                            if static is not None: zt_cov = torch.cat([zt_cov,static],axis=-1)
                            else: zt_cov = torch.cat([zt_cov],axis=-1)
            ...
            if train_toxic is not None:
                ...
                if loss_output_batch != 0  and loss_a_batch != 0:
                    loss_dec= loss_output_batch + mu * loss_a_batch
                else:
                    loss_dec=torch.Tensor([np.nan])
                ...
                if dec_expand:
                    if sofa_expand:
                        ...
                        if med_dec_start:
                            ...
                            if static_val is not None: zt_val = torch.cat([zt_val,sofa_start,static_val,treat_start,treat_past],axis=-1)
                            else: zt_val = torch.cat([zt_val,sofa_start,treat_start,treat_past],axis=-1)
                        else:
                            if static_val is not None: zt_val = torch.cat([zt_val,sofa_start,static_val],axis=-1)
                            else: zt_val = torch.cat([zt_val,sofa_start],axis=-1)
                    else:
                       if static_val is not None:  zt_val = torch.cat([zt_val,static_val],axis=-1)
                       else: zt_val = torch.cat([zt_val],axis=-1)
        ...
        if validation_toxic is not None:
            ...
            if loss_output_val != 0 and loss_a_val != 0:
                loss_val= loss_output_val +  mu * loss_a_val
            else:
                loss_val=torch.Tensor([np.nan])
        print('Epoch: {}   Validation loss: {}'.format(epoch, loss_val.item()))
                ...
                for i in range(treat_past.shape[0]):
                    for j in range(treat_past.shape[2]):
                        ...
                        if val.numel()>0:
                            ...
                            treat_past[i,0,j] = 0
                if static is not None: zt_val = torch.cat([zt_val,sofa_start,static,treat_start,treat_past],axis=-1)
                else: zt_val = torch.cat([zt_val,sofa_start,treat_start,treat_past],axis=-1)
            else:
                if static is not None: zt_val = torch.cat([zt_val,sofa_start,static],axis=-1)
                else: zt_val = torch.cat([zt_val,sofa_start],axis=-1)
        else:
            if static is not None: zt_val = torch.cat([zt_val,static],axis=-1)
            else:  zt_val = torch.cat([zt_val],axis=-1)
...
def prediction_measures_decoder(model, model_decoder, validation_output, validation_toxic, validation_treatments, covariables, time_covariates, active_entries, static=None, offset=5,max_horizon=5,unscaled=False, rectilinear_index=None, step=None, variables_std=None,variables_mean=None,variables=None, dec_expand=True,sofa_expand=True, med_dec=False, med_dec_start=True):
    ...
    if X_toxic is not None: X_toxic_std=X_toxic.clone()
    ...
    if X_toxic is not None:
        ...
        if step is None:
            dat_sd = torch.from_numpy(np.nanstd(torch.cat([X_std[:,offset+1:offset+max_horizon+1,:]],dim=-1).cpu(),axis=0)).to(model.device)
            nrmse_sd = rmse/dat_sd
            
            dat_mean = torch.from_numpy(np.nanmean(torch.cat([X_std[:,offset+1:offset+max_horizon+1,:]],dim=-1).cpu(),axis=0)).to(model.device)
            nrmse_mean = rmse/dat_mean
            
            dat_iqr = torch.from_numpy(np.nanquantile(torch.cat([X_std[:,offset+1:offset+max_horizon+1,:]],dim=-1).cpu(),q=0.75,axis=0)).to(model.device) - torch.from_numpy(np.nanquantile(X_std[:,offset+1:offset+max_horizon+1,:].cpu(),q=0.25,axis=0)).to(model.device)
            nrmse_iqr=rmse/dat_iqr
        else:
            for i in range(0,mse.shape[0],step):
                dat_sd = torch.from_numpy(np.nanstd(torch.cat([X_std[:,offset+1+i:offset+1+i+step,:]],dim=-1).cpu(),axis=(0,1))).to(model.device)
                dat_mean = torch.from_numpy(np.nanmean(torch.cat([X_std[:,offset+1+i:offset+1+i+step,:]],dim=-1).cpu(),axis=(0,1))).to(model.device)
                dat_iqr = torch.from_numpy(np.nanquantile(torch.cat([X_std[:,offset+1+i:offset+1+i+step,:]],dim=-1).cpu(),q=0.75,axis=(0,1))).to(model.device) - torch.from_numpy(np.nanquantile(torch.cat([X_std[:,offset+1+i:offset+1+i+step,:],X_toxic_std[:,offset+1+i:offset+1+i+step,:]],dim=-1).cpu(),q=0.25,axis=(0,1))).to(model.device)
        ...
        if step is None:
            for i in range(mape.shape[0]):
                mape[i,0] = mapeloss(pred_output_val[:,i,0:1][(active_entries[:,offset+i+2,:].bool()) & (~X[:,offset+i+1,:].isnan())], X[:,offset+i+1,:][(active_entries[:,offset+2+i,:].bool()) & (~X[:,offset+i+1,:].isnan())])
                for j in range(mape.shape[1]-1):
                    mape[i,j+1] = mapeloss(pred_output_val[:,i,j+1:j+2][(active_entries[:,offset+i+2,:].bool())])
        else:
            for i in range(0,mape.shape[0],step):
                mape[i,0] = mapeloss(pred_output_val[:,i:i+step,0:1][(active_entries[:,offset+i+2:min(offset+2+i+step,offset+2+mse.shape[0]),:].bool()) & (~X[:,offset+i+1:min(offset+1+i+step,offset+1+mse.shape[0]),:].isnan())], X[:,offset+i+1:min(offset+1+i+step,offset+1+mse.shape[0]),:][(active_entries[:,offset+2+i:min(offset+2+i+step,offset+2+mse.shape[0])].bool()) & (~X[:,offset+i+1:min(offset+1+i+step,offset+1+mse.shape[0]),:].isnan())])
                for j in range(mape.shape[1]-1):
                    mape[i,j+1] = mapeloss(pred_output_val[:,i:i+step,j+1:j+2][(active_entries[:,offset+i+2:min(offset+2+i+step,offset+2+mse.shape[0]),:].bool())])
        ...
        if step is None:
            for i in range(wape.shape[0]):
                wape[i,0] = wapeloss(pred_output_val[:,i,0:1][(active_entries[:,offset+i+2,:].bool()) & (~X[:,offset+i+1,:].isnan())], X[:,offset+i+1,:][(active_entries[:,offset+2+i,:].bool()) & (~X[:,offset+i+1,:].isnan())])
                for j in range(wape.shape[1]-1):
                    wape[i,j+1] = wapeloss(pred_output_val[:,i,j+1:j+2][(active_entries[:,offset+i+2,:].bool())])
        else:
            for i in range(0,wape.shape[0],step):
                wape[i,0] = wapeloss(pred_output_val[:,i:i+step,0:1][(active_entries[:,offset+i+2:min(offset+2+i+step,offset+2+mse.shape[0]),:].bool()) & (~X[:,offset+i+1:min(offset+1+i+step,offset+1+mse.shape[0]),:].isnan())], X[:,offset+i+1:min(offset+1+i+step,offset+1+mse.shape[0]),:][(active_entries[:,offset+2+i:min(offset+2+i+step,offset+2+mse.shape[0])].bool()) & (~X[:,offset+i+1:min(offset+1+i+step,offset+1+mse.shape[0]),:].isnan())])
                for j in range(wape.shape[1]-1):
                    wape[i,j+1] = wapeloss(pred_output_val[:,i:i+step,j+1:j+2][(active_entries[:,offset+i+2:min(offset+2+i+step,offset+2+mse.shape[0]),:].bool())])
...
def heatmap_pred_dec(model, model_decoder, validation_output, validation_toxic, validation_treatments, covariables, time_covariates, active_entries, static=None, index=0, offset=0, max_horizon=9,loss='rmse', rectilinear_index=None, step=None, unscaled=False, variables_std=None,variables_mean=None,variables=None,title=str(),save_link=None,save_map=None,load_map=None,vmin=None,vmax=None,colorbar=True,diagmultistep=False, dec_expand=True,sofa_expand=True, med_dec=False, med_dec_start=True, invert=False,heat_max=None):
    ...
    if loss=='rmse':
        ...
        # if index == 0: ax.set_title(title+'Sofa-Score Root Mean Square Error')
        # elif index == 1: ax.set_title(title+'Cratinin Root Mean Square Error')
        # elif index == 2: ax.set_title(title+'Bilirubin total Root Mean Square Error')
        # elif index == 3: ax.set_title(title+'ALT Root Mean Square Error')
        # else: ax.set_title('Root Mean Square Error')
        ax.set_title(title+'Root Mean Square Error')
    elif loss=='mse':
        ax.set_title('Mean Square Error',fontsize= 13)
    elif loss=='mae':
