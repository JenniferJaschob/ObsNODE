...
#import to_absolute_path
from hydra.utils import instantiate, to_absolute_path
from src import ROOT_PATH
from src.data.dataset_collection import RealDatasetCollection
from src.data.mimic_iii.load_data import load_mimic3_data_processed, process_static_features
...
class MIMIC3RealDatasetCollection(RealDatasetCollection):
    ...
    def __init__(self,
        ...
        all_vitals = pd.read_csv(to_absolute_path("runnables/x_all_dynamic.csv"))
        all_vitals = all_vitals.rename(columns={'time': 'hours_in'})
        all_vitals = all_vitals.set_index(['subject_id', 'hours_in'])
        static_features = pd.read_csv(to_absolute_path("runnables/x_all_static.csv"), index_col="subject_id")

        all_vitals = all_vitals.fillna(method='ffill')
        all_vitals = all_vitals.fillna(method='bfill')


        # subject_id,time,SOFA,creatinine,bilirubin_total,alt,index,aniongap,bicarbonate,bun,dbp,platelet,rdw,sbp,SOFA_mask,aniongap_mask,bicarbonate_mask,bun_mask,creatinine_mask,dbp_mask,platelet_mask,rdw_mask,sbp_mask,bilirubin_total_mask,alt_mask
        # subject_id,admission_age,height,weight,male,admission_age_mask,height_mask,weight_mask,male_mask,Vancomycinstat,Piperacillin-Tazobactamstat,Ceftriaxonstat

        outcome_list = ["SOFA", "creatinine", "bilirubin_total", "alt"]
        treatment_list = ["Vancomycinstat", "Piperacillin-Tazobactamstat", "Ceftriaxonstat"]
        vital_list = ["SOFA", "creatinine", "bilirubin_total", "alt", "index", "aniongap", "bicarbonate", "bun", "dbp", "platelet", "rdw", "sbp", "SOFA_mask", "aniongap_mask",
                      "bicarbonate_mask", "bun_mask", "creatinine_mask", "dbp_mask", "platelet_mask", "rdw_mask",
                      "sbp_mask", "bilirubin_total_mask", "alt_mask"]
        one_hot_static_features = ["male","admission_age_mask", "height_mask", "weight_mask", "male_mask"]

        # Split treatment from static features
        treatments = static_features[treatment_list].copy()
        static_features = static_features.drop(columns=treatment_list)
        static_features[one_hot_static_features] = static_features[one_hot_static_features].astype(int)
        logger.info(f'Shape of treatments: {treatments.shape}')
        logger.info(f'Shape of static: {static_features.shape}')
        logger.info(f'Shape of vitals: {all_vitals.shape}')
        
        # Filtering longer then min_seq_length and cropping to max_seq_length and 
        # random selection of patients of size max_number
        ...
        # Expand treatments to match the temporal structure of vitals
        # Create a MultiIndex DataFrame for treatments matching vitals structure
        treatments_expanded = []
        for subject_id in all_vitals.index.get_level_values('subject_id').unique():
            subject_vitals = all_vitals.loc[subject_id]
            subject_treatments = treatments.loc[subject_id]
            # Repeat treatment values for each timestep of this subject
            subject_treatments_expanded = pd.DataFrame(
                np.tile(subject_treatments.values, (len(subject_vitals), 1)),
                columns=treatment_list,
                index=subject_vitals.index
            )
            # Add subject_id as first level of MultiIndex
            subject_treatments_expanded.index = pd.MultiIndex.from_product(
                [[subject_id], subject_treatments_expanded.index],
                names=['subject_id', 'hours_in']
            )
            treatments_expanded.append(subject_treatments_expanded)
        treatments = pd.concat(treatments_expanded)
        logger.info(f'Shape of expanded treatments: {treatments.shape}')

        # Ohne standardisierung, 
        # TODO: Unscaled data instead of already scaled data.
        ...
        # data_toxic_test_unscaled[:,:,0] = data_toxic_test_unscaled[:,:,0]*variables_std[variables.index('SOFA')]+variables_mean[variables.index('SOFA')]
