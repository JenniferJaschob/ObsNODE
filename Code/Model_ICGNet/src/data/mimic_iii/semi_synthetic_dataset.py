...
class MIMIC3SyntheticDataset(MIMIC3RealDataset):
    ...
    def __init__(self,
        ...
        if mode == 'factual':
            ...
            if mode == 'counterfactual_one_step':
                ...
                if self.treatments_seq is None:
    ...
    def treat_patient_counterfactually(self, patient_ix: int, seed: int = None):
        ...
        """
        patient_df = self.all_vitals.loc[patient_ix].copy()
        patient_ae = (~patient_df.copy().drop(columns=['fact']).isna().all(1)).astype(float)
        # Counterfactual sequence starts at last active entry minus projection horizon
        patient_cf_start = patient_ae.sum()-1-self.projection_horizon #patient_ae.sum()-1 - self.projection_horizon
...
class MIMIC3SyntheticDatasetCollection(SyntheticDatasetCollection):
    ...
    def __init__(self,
        ...
        # Nur manche features behalten
        selected_features_vitals = [
            "SOFA",
            "creatinine",
            "bilirubin_total",
            "alt"
        ]
        all_vitals = all_vitals[selected_features_vitals]
        selected_features_static = [
            "Vancomycinstat",
            "Piperacillin-Tazobactamstat",
            "Ceftriaxonstat"
        ]
        static_features = static_features[selected_features_static]


        # NUR DIE ERSTEN n_patients PATIENTEN NUTZEN (zum Testen)
        n_patients = 100
        patient_ids = static_features.index[:n_patients]
        static_features = static_features.loc[patient_ids]
        all_vitals = all_vitals.loc[patient_ids]
        ...
        self.test_cf_treatment_seq = \
            MIMIC3SyntheticDataset(all_vitals_test, static_features_test, synth_outcomes_list, synth_treatments_list,
                                   ...
                                   projection_horizon, n_treatments_seq=1 if treatment_sequence is not None else n_treatments_seq,
                                   treatments_seq=np.array([treatment_sequence]) if treatment_sequence is not None else None)
