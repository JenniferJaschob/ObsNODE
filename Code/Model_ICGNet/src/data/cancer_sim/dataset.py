...
from runnables.train_gtransformer import read_csv_to_dict
...
from src.data.cancer_sim.cancer_simulation import generate_params, get_scaling_params, simulate_factual, \
    simulate_counterfactual_1_step, simulate_counterfactuals_treatment_seq, get_scaling_params_with_toxicity
...
class SyntheticCancerDataset(Dataset):
    ...
    def __init__(self,
                 ...
                 cf_treatment_sequence: list = None,
                 manual: bool = False):
        """
        Args:
        ...
        if(not manual):
            ...
            if mode == 'factual':
                ...
                self.data = simulate_counterfactuals_treatment_seq(self.params, seq_length, projection_horizon, cf_seq_mode, cf_treatment_sequence)
        else:
            self.data = {}
    ...
    def process_data(self, scaling_params):
        ...
        if not self.processed:
            ...
            if(not self.manual):
                ...
                input_means = mean[['cancer_volume', 'toxicity', 'chemo_application', 'radio_application']].values.flatten()
                input_stds = std[['cancer_volume', 'toxicity', 'chemo_application', 'radio_application']].values.flatten()
            ...
            if(not self.manual):
                ...
                toxicity = (self.data['toxicity'] - mean['toxicity']) / std['toxicity']
            ...
            if(not self.manual):
                current_covariates = np.concatenate([cancer_volume[:, :-offset, np.newaxis], 
                                                     patient_types[:, :-offset, np.newaxis]],axis=-1)
            else:                                         
                current_covariates = np.concatenate([cancer_volume[:, :-offset, np.newaxis], 
                                                     toxicity[:, :-offset, np.newaxis]],axis=-1)
            ...
            if(not self.manual):
                ...
                outputs = np.concatenate([
                    cancer_volume[:, horizon:, np.newaxis],    # Tumorvolumen
                    toxicity[:, horizon:, np.newaxis]          # Toxicity
                ], axis=-1)

                output_means = mean[['cancer_volume', 'toxicity']].values.flatten()
                output_stds = std[['cancer_volume', 'toxicity']].values.flatten()

                # Add active entries

                active_entries = np.zeros(outputs.shape[:-1] + (1,))  # (batch, seq, 1)
                for i in range(sequence_lengths.shape[0]):
                    sequence_length = int(sequence_lengths[i])
                    active_entries[i, :sequence_length, 0] = 1
                ...
                # Unscaled outputs für beide Variablen
                self.data['unscaled_outputs'] = np.concatenate([
                    outputs[..., 0:1] * std['cancer_volume'] + mean['cancer_volume'],
                    outputs[..., 1:2] * std['toxicity'] + mean['toxicity']
                ], axis=-1)
            ...
            if not self.manual:
                ...
                # Beide Outcomes als prev_outputs (Tumorvolumen und Toxizität)
                self.data['prev_outputs'] = current_covariates[:, :, :2]
    ...
    def explode_trajectories(self, projection_horizon):
        ...
        for i in range(num_patients):
            ...
            for t in range(projection_horizon, sequence_length):  # shift outputs back to align with inputs
    ...
    def process_sequential_multi(self, projection_horizon):
        ...
        if not self.processed_autoregressive:
            ...
            # fixed split marks first index of future
            # in CF sequence, curr_treatments[fixed_split:(fixed_split+projection_horizon)] is counterfactual treatment sequence
            # active_entries[:(fixed_split+projection_horizon)] is 1, later points are 0
            self.processed_autoregressive = True
...
class SyntheticCancerDatasetCollection(SyntheticDatasetCollection):
    ...
    def __init__(self,
        ...
        self.train_f = SyntheticCancerDataset(chemo_coeff, radio_coeff, num_patients['train'], window_size, max_seq_length,
                                              'train', lag=lag, treatment_mode=treatment_mode, manual=self.manual)

        self.val_f = SyntheticCancerDataset(chemo_coeff, radio_coeff, num_patients['val'], window_size, max_seq_length, 'val',
                                            lag=lag, treatment_mode=treatment_mode,manual=self.manual)

        self.test_f = SyntheticCancerDataset(chemo_coeff, radio_coeff, num_patients['test'], window_size,
                                                       max_seq_length, 'test', mode='counterfactual_one_step', lag=lag,
                                                       treatment_mode=treatment_mode, manual=self.manual,
                                                       )

        self.test_cf_treatment_seq = SyntheticCancerDataset(chemo_coeff, radio_coeff, num_patients['test'], window_size,
                                                            max_seq_length, 'test_cf_treat', mode='counterfactual_treatment_seq',
                                                            projection_horizon=projection_horizon, lag=lag,
                                                            cf_seq_mode=cf_seq_mode, treatment_mode=treatment_mode,
                                                            cf_treatment_sequence=treatment_sequence, manual=self.manual)
        #inject data dicts
        if(self.manual):
            train_data = read_csv_to_dict(to_absolute_path("runnables/cancer_data_train_val/train_cancer_DoseAI_data.csv"))
            val_data = read_csv_to_dict(to_absolute_path("runnables/cancer_data_train_val/val_cancer_DoseAI_data.csv"))
            test_data = read_csv_to_dict(to_absolute_path(f"runnables/cancer_data_all_gamma_CSV/test_cancer_gamma_{int(chemo_coeff)}_data.csv"))
            test_cf_seq_data = read_csv_to_dict(to_absolute_path(f"runnables/cancer_data_all_gamma_CSV/test_cancer_gamma_{int(chemo_coeff)}_data.csv"))

            self.train_f.data = train_data
            self.val_f.data = val_data
            self.test_f.data = test_data
            self.test_cf_treatment_seq.data = test_cf_seq_data

        self.projection_horizon = projection_horizon
        self.autoregressive = True
        self.has_vitals = False
        #self.train_scaling_params = self.train_f.get_scaling_params()
        self.train_scaling_params = get_scaling_params_with_toxicity(self.train_f.data) if self.manual else self.train_f.get_scaling_params()
        print("SCALING_PARAMS:", self.train_scaling_params)