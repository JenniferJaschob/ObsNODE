#Adaptation from https://github.com/konstantinhess/IGC_net/blob/main/src/models/time_varying_model.py
#Konstantin Hess et al., "IGC-Net for conditional average potential outcome estimation over time", in The Fourteenth International Conference on Learning Representations", 
#Published as a conference paper at ICLR 2026 doi:  https://doi.org/10.48550/arXiv.2405.21012
    ...
    def finetune(self, resources_per_trial: dict):
        ...
        ray.init(num_gpus=len(eval(str(self.hparams.exp.gpus))), num_cpus=4, include_dashboard=False,
                 ...
                 runtime_env={'env_vars': {
                        'PYTHONPATH': str(pathlib.Path(__file__).parent.parent.parent.absolute()),
        ...
        # Get the best validation loss from the analysis results
        # The key is the same as the 'metric' used in tune.run
        best_val_loss = analysis.best_result[f"val_{self.tuning_criterion}_all"]
        
        # Add the validation loss to the dictionary that will be returned
        best_config['val_loss'] = best_val_loss

        # Save the best hyperparameters to a text file
        with open(to_absolute_path("RayTuneOutput/hyperparameter_optimization_results.txt"), "w") as f:
        ...
        self.set_hparams(self.hparams.model, best_config, self.input_size, self.model_type)
