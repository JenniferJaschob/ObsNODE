#Adaptation from https://github.com/konstantinhess/IGC_net/blob/main/src/models/gt.py
...
class GT(TimeVaryingCausalModel):
    ...
    def forward(self, batch):
        ...
        if not self.dataset.name == "tumor_generator":
            static_features = batch['static_features']
        else:
            static_features = torch.zeros((batch['current_covariates'].shape[0], 0), device=batch['current_covariates'].device)
        ...
        # 3) Prediction: only use the first head ("=outermost expectation")
            ...
            fixed_split = batch['sequence_lengths'] - 1 if self.projection_horizon > 0 else batch['sequence_lengths']
