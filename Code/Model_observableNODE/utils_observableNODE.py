...
def float_data(data):
    """
    Converts all input tensors in a tuple to float type.

    Args:
        data (tuple): A tuple of three tensors (t_data_all, x_data, y_data).

    Returns:
        tuple: The same three tensors converted to torch.float.
    """
...
def last_index(data_all):
    """
    Extracts the last valid (non-NaN) value for each feature in a tensor.

    Args:
        data_all (torch.Tensor): 
            Tensor of shape (timepoints, number of patients, number of covariables) 
            or (timepoints, number of patients)
                                 May contain NaN values.

    Returns:
        torch.Tensor: Tensor of shape (batch, features) containing the last valid values
                      along the time dimension (NaN if all values are missing).
    """
    ...
    for i in range(data_all.size(-1)):
        ...
        # flip + argmax to locate the last True in mask for each batch
        ...
        # If all values are NaN for a batch, keep result NaN
...
def def_x_obs(x_obs,time_obs=None):
    """
    Prepares observation data by adding a feature dimension and optionally 
    concatenating time observations.
    Args:
        x_obs (torch.Tensor): 
            Observation data of shape (timepoints, number of patients, number of covariables) 
            or (timepoints, number of patients)
        time_obs (torch.Tensor, optional): 
            Time data of shape (timepoints, number of patients, 1) 

    Returns:
        torch.Tensor: 
            Tensor with shape (timepoints, number of patients, number of covariables [+1 if time_obs is included]).
    """
    if x_obs.dim() == 2:
...
class MyObservableNeuralODE_withNNs(nn.Module):
    """
    Observable Neural ODE model with optional helper neural networks.
    
    This class defines a neural Ordinary Differential Equation (ODE)
    model that can include additional neural networks ("helperNNs") to define 
    the observed normal form
    
    Args:
        helperNN_list (list[nn.Module], optional): List of helper neural networks of observed normal form
        activation (str): Activation function 
        hidden_sizes (int): Number of neurons per hidden layer
        num_layers (int): Number of hidden layers 
        n_x (int):  Dimension of the observable state variable x.
        n_z (int): Dimension of the latent variable z.
        u (torch.Tensor, optional): Control input tensor (time-dependent external signal).
        n_u (int):  Dimension of the control input.
    
    Attributes:
        net (nn.Sequential): 
            Feedforward neural network mapping concatenated inputs to outputs.
    """
    ...
    def forward(self,t,z):
        """
        Forward pass through the Observable Neural ODE.
        
        Args:
            t (float or torch.Tensor): 
                Current time
            z (torch.Tensor): 
                Latent state tensor of shape (timepoints, number of patients, number of covariables)
        
        Returns:
            torch.Tensor: 
                Updated state tensor combining transformed latent variables 
                of shape (timepoints, number of patients, output_size)
        """
...
class MyhelperNN(nn.Module):
    ...
    """
    Helper neural network module for use within the Observable Neural ODE framework.
    
    This network maps an input latent vector (and optionally a control input `u`)
    to an output vector, typically representing corrections or transformations
    for intermediate latent states in the larger ODE system.
    
    Args:
        activation (str): Activation function
        hidden_sizes (int): Number of neurons per hidden layer
        num_layers (int): Number of hidden layers in the network
        n_z_i (int): Size of the latent input vector `z_i`.
        u (torch.Tensor, optional): Optional control input tensor.
        n_u (int): Dimension of the control input (if used).
        output_size (int): Dimension of the network output.
    
    Attributes:
        net (nn.Sequential): The constructed feedforward neural network.
    """
    ...
    def forward(self,t,z_i):
        """
        Forward pass through the helper network.
        
        Args:
            t (float or torch.Tensor): Current time
            z_i (torch.Tensor): Input latent state tensor
        
        Returns:
            torch.Tensor: 
                Output tensor of shape (timepoints, number of patients, output_size)
        """
...
def initialize_imputation(X, W):
    """
    Initialize missing value imputation for a dataset.
    
    Args:
        X (torch.Tensor): Input data tensor
        W (torch.Tensor): Binary mask tensor of the same shape as X, 
                          where 1 indicates observed values and 0 indicates missing values.
    
    Returns:
        torch.Tensor: Initial imputed values per feature (A) for missing data.
    """
...
class Vaderlayer(nn.Module):
    """
    Learnable imputation layer that fills missing values with trainable parameters

    Attributes:
        weights (nn.Parameter): Trainable initial imputed values for each feature
    
    Fill missing entries in X using the learnable weights

    Args:
        X (torch.Tensor): Input tensor with potential NaNs
        W (torch.Tensor): Binary mask tensor (1 = observed, 0 = missing)

    Returns:
        torch.Tensor: Tensor with missing values replaced by trainable weights
    """
...
class LSTMRecognitionModel_with_nan(nn.Module):
    """
    LSTM-based recognition model that handles missing (NaN) values in time series data.

    This model processes observed sequences that may contain missing values (NaNs).
    It first imputes missing data using a VADER-like layer, then encodes the sequence 
    using an LSTM and outputs a latent representation combined with the last observed time feature.
    ...
    Args:
        ...
        n_t (int): 
            Dimension of time features included in the input.
        hidden_dim (int): 
            Number of hidden units in the LSTM.
        num_layers (int): 
            Number of stacked LSTM layers.
        padding_value (float): 
            Value representing missing data (default: NaN).
        W_train (torch.Tensor, optional): 
            Optional weight matrix used during training (not directly applied here).
        dropout (float): 
            Dropout rate applied between LSTM layers.

    Attributes:
        lstm (nn.LSTM): 
            LSTM network for sequence encoding.
        fc (nn.Linear): 
            Fully connected layer mapping LSTM outputs to latent dimensions.
    """
    ...
    def forward(self, x_obs):
        ...
        Args:
            x_obs (torch.Tensor): 
                Observed input tensor of shape (timepoints, number of patients, number of covariables)
                or (timepoints, number of patients) 
        
        Returns:
            torch.Tensor: 
                Concatenated tensor with initial state of z 
                of shape (number of patients, n_t + (n_z-1)*n_x)
...
class MyModel_adjoint(nn.Module): 
    ...
    """
    Neural ODE model using the adjoint method for gradient computation.
    
    Attributes:
        Node (nn.Module): The neural network representing the dynamics (right-hand side of the ODE)
        Observer (nn.Module): Maps observed data x_obs to the initial latent state z for the ODE
    
    Forward Inputs:
        time (torch.Tensor): Sequence of time points at which to solve the ODE
        x_obs (torch.Tensor): Observed data input used to initialize the latent state
        method (str): ODE solver method 
        options (dict, optional): Additional solver options
    
    Forward Output:
        z_pred (torch.Tensor): Predicted latent states
...
def loss_function_1D(x_true, t_true, x_pred, t_pred,loss_op='default'):
    ...
    """
    Computes the loss for a single 1D sequence, handling NaN values in the true data.
    
    Inputs:
        x_true (torch.Tensor): True sequence values
        t_true (torch.Tensor): Corresponding time points for x_true
        x_pred (torch.Tensor): Predicted sequence values
        t_pred (torch.Tensor): Time points for predictions
        loss_op (str): Loss type
    
    Output:
        out (torch.Tensor): Scalar tensor representing the mean loss over the batch.
    """
...
def loss_function_sum(x_true, t_true, x_pred, t_pred,loss_op='default',n_x_dach = None,w= 0):
    ...
    """
    Computes the loss across multiple output dimensions
    
    Inputs:
        x_true (torch.Tensor): True sequences
        t_true (torch.Tensor): Corresponding time points
        x_pred (torch.Tensor): Predicted sequences
        t_pred (torch.Tensor): Predicted time points 
        loss_op (str): Loss type for each 1D sequence
        n_x_dach (int, optional): Number of main output dimensions to compute primary loss over
        w (float): Weight for additional output dimensions beyond n_x_dach
    
    Output:
        out (torch.Tensor): Scalar tensor representing the mean loss across all output dimensions
    """
...
def handle_early_stopping(val_loss, best_val_loss_early_stopping, patience, early_stopping_counter, min_delta):
    """
    Checks whether training should stop early based on validation loss improvements
    
    Inputs:
        val_loss (float): Current validation loss
        best_val_loss_early_stopping (float): Best validation loss
        patience (int): Number of epochs to wait for improvement before stopping
        early_stopping_counter (int): Counter for epochs without improvement
        min_delta (float): Minimum required improvement to reset counter
    
    Output:
        stop (bool): True if early stopping criterion is met
        best_val_loss_early_stopping (float): Updated best validation loss
        early_stopping_counter (int): Updated counter of epochs without improvement
    """
...
def data_time_step(x,step_size):
    """
    Downsamples or inserts time steps into a sequence with NaNs for missing steps
    
    Inputs:
        x (torch.Tensor): Input tensor with shape (timepoints, number of patients, number of covariables)
        step_size (float): Fraction of time steps to keep
    
    Output:
        x_expanded (torch.Tensor): Tensor of reduced time steps with NaNs where data is missing
    """
    T, B, F = x.shape
...
def train_diff_ts(model_node,model_observer,x_train,x_val,t_train,t_val,u_train=None,u_val=None,x_train_static=None,x_val_static=None, time_obs_pred=False,
    ...
    """
            ...
            Parameters
            ----------
            model_node : (torch.nn.Module) The dynamic model describing the temporal evolution of the system
            model_observer : (torch.nn.Module) Observer model mapping states to measurable quantities
            x_train : (torch.Tensor) Input training data.
            x_val : (torch.Tensor) Input or state validation data
            t_train : (torch.Tensor) Time points corresponding to training data
            t_val : (torch.Tensor) Time points corresponding to validation data
            u_train : (torch.Tensor, (default: None)) Control or input signals for training 
            u_val : (torch.Tensor, (default: None)) Control signals for validation 
            time_obs_pred : (bool, (default: False)) Whether to include time-dependent observations for prediction 
            list_index_t_s : (list of int, (default: [10])) List of time indices at which predictions are evaluated 
            method : (str, optional) Numerical integration method
            options : (dict, (default: None)) Additional options for the integrator 
            optimizer_func : (str, (default: 'Adam')) Optimizer to use for training 
            ...
            min_delta : (float, (default: 0.005))  Minimum improvement for early stopping 
            patience : (int, (default: 10)) Number of epochs without improvement before stopping 
            step_size : (float, (default: 1.0) Step size for integration or scheduler 
            save_results : (bool, (default: False)) Whether to save trained models 
            save_path : (str,optional) File path to save results 
            time_multidim : (array-like, (default: None)) Time dimensions for multidimensional time series 
            loss_op : (str, optional) Loss function default: 'default')
            hyperopt : (bool, (default: False)) Whether to perform hyperparameter optimization 
            n_x_dach : (int, (default: None)) Dimension of estimated states or predictions 
            w : (float, (default: 0)) Weighting factor for certain loss components 
            data_step : (bool, (default: False))  Whether to process data stepwise 
            step_pred : (bool,(default: False) Whether predictions are made step-by-step 
            num_pred : (int, (default: 2)) Number of time steps to predict 
        
            Returns
            -------
            dict
                Dictionary containing training history, validation losses, and optionally saved model paths
        
            Notes
            -----
            - `device` is automatically set from `x_train.device` for GPU/CPU consistency
            - Supports both static and time-dependent features, with optional control inputs
    """
...
def predict(model, t_before, x_before, t_after, x_after, method='euler',options=None, step_size=0.1,u_before=None,time_obs_pred = False,index_t_s=10,loss_op='default',n_x_dach=None,w=0,data_step=False):#, device
    """
    Predicts future states of a dynamical system using the provided model.

    Args:
        model: Neural ODE model 
        t_before (torch.Tensor): Time points of known observations up to index_t_s
        x_before (torch.Tensor): Observed values up to index_t_s
        t_after (torch.Tensor): Time points for prediction after index_t_s
        x_after (torch.Tensor): Ground truth values for comparison after index_t_s
        method (str): Integration method for NODE
        options (dict): Optional integration parameters
        step_size (float): Step size for prediction time discretization
        u_before (torch.Tensor, optional): Control signal
        time_obs_pred (bool): Whether to include time observations in model input
        index_t_s (int): Starting index for prediction
        loss_op (str): Type of loss to compute 
        n_x_dach (int, optional): Dimension of x
        w (float): Weight for additional outputs in loss
        data_step (bool): Whether to expand data to step size

    Returns:
        x_pred (torch.Tensor): Predicted values
        t_pred (torch.Tensor): Prediction time points
        z_pred (torch.Tensor): Latent states from model
        loss (torch.Tensor): Computed loss for predictions
    """    
...
def step_predict(model, x_test, t_test,u_test, index_t_s,num_pred=2, method='euler',options=None, step_size=0.1,time_obs_pred = False,loss_op='default',n_x_dach=None,w=0,data_step=False,training=False):
    """
       Performs step-wise predictions over a sequence, using previous predictions for future steps.
    
       Args:
           model: Neural ODE model
           x_test (torch.Tensor): Observed sequence for testing
           t_test (torch.Tensor): Time points of test observations
           u_test (torch.Tensor): Control signal
           index_t_s (int): Starting index for prediction
           num_pred (int): Number of steps to predict per iteration
           training (bool): Whether in training mode (affects loss computation)
           method, options, step_size, time_obs_pred, loss_op, n_x_dach, w, data_step: see `predict`
    
       Returns:
           x_pred_all (torch.Tensor): All predicted outputs concatenated
           t_pred_all (torch.Tensor): All prediction time points concatenated
           z_pred_all (torch.Tensor): All latent states concatenated
           loss_out (torch.Tensor): Mean loss over all step-wise predictions
       """
...
def heatmap_pred(x_test=None,t_test=None,u_test=None, list_index_t_s_pred=None,model=None,n_x_dach=2, w=0,step_size=1,time_obs_pred=False,method='rk4',st=False,x_train=False,pred_func = 'new',
                ...
                model_decoder=None, test_X=None, test_toxic=None, test_treatments=None, covariables_x_test=None, test_time_covariates=None, active_entries_test=None,  a_loss=None,static=None,dataset = None, val_seed =None, step_pred=False,data_step=1.0,heat_max=None, model_name = None):
    ...
    if invert:
        ...
        elif dataset == 'mimic4':ax.set_ylabel('Observed until hour y',fontsize= 13)
        elif dataset == 'semisynMimic4':ax.set_ylabel('Observed until hour y',fontsize= 13)
        else: ax.set_ylabel('Observed until hour y',fontsize= 13)
        
        if dataset =='DoseAI':ax.set_xlabel('Forecast horizon in treatment cycles',fontsize= 13)
        elif dataset == 'mimic4':ax.set_xlabel('Forecast horizon in hours',fontsize= 13)
        elif dataset == 'semisynMimic4':ax.set_xlabel('Forecast horizon in hours',fontsize= 13)
        else: ax.set_xlabel('Forecast horizon in hours',fontsize= 13)
    
    else:
        if dataset == 'DoseAI':ax.set_xlabel('Observed until treatment cycle x in months',fontsize= 13)
        elif dataset == 'mimic4':ax.set_xlabel('Observed until hour x',fontsize= 13)
        elif dataset == 'semisynMimic4':ax.set_xlabel('Observed until hour x',fontsize= 13)
        else: ax.set_xlabel('Observed until hour x',fontsize= 13)
        
        if dataset == 'DoseAI':ax.set_ylabel('Forecast horizon in treatment cycles',fontsize= 13)
        elif dataset == 'mimic4':ax.set_ylabel('Forecast horizon in hours',fontsize= 13)
        elif dataset == 'semisynMimic4':ax.set_ylabel('Forecast horizon in hours',fontsize= 13)
        else: ax.set_ylabel('Forecast horizon in hours',fontsize= 13)
