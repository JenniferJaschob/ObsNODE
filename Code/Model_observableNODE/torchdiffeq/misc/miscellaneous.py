...
import torch
import warnings
from event_handling.event_handling import combine_event_functions
...
def check_inputs_adjoint(node_func, obs_func, y_obs, t, rtol, atol, method, options, event_fn, SOLVERS):
    ...
    if t_is_reversed:
        ...
        # Ensure time values are un-negated when calling functions.
        node_func = _ReverseFunc(node_func, mul=-1.0)
        if event_fn is not None:
            event_fn = _ReverseFunc(event_fn)
    ...
    # Add perturb argument to func.
    node_func = _PerturbFunc(node_func)

    return shapes, node_func, obs_func, y_obs, t, rtol, atol, method, options, event_fn, t_is_reversed
